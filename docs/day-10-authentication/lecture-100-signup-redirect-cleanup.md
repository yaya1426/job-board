# Lecture 100 - Signup Redirect on Success | تحسين إعادة التوجيه بعد التسجيل
...
1. Refactor `SignupForm` to Pattern B plain async handler (not `useActionState`).
2. Read `email` and `password` from `FormData` once; reuse for `signIn` after signup.
3. Flow: `await handleSignup(formData)` → on errors, `setErrors` → on success, `signIn("credentials", { email, password, redirect: false })`.
4. On sign-in success: `router.push(callbackUrl ?? "/")` then `router.refresh()`.
5. Keep `handleSignup` as a plain async function — no `prevState`, no `redirect()` inside the action.
6. Test with `callbackUrl=/jobs/[id]` from apply prompt (fully wired in Lecture 104).
...
## Next
[Lecture 101 — Getting Current Logged User](./lecture-101-current-user.md) introduces `getCurrentUser()` for server-side identity.
