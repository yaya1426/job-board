...
1. Show a Mongoose document in the debugger — point at `_id` and `posted`.
2. Create `toJob` and explain the Next.js serialization boundary.
3. Move existing job reads/writes from the service into the repository.
4. Update `jobs.service.ts` to import only from `repositories/jobs.repository.ts`.
