# Lecture 124.1 - Supplementary: Docker Build Environment Variables | متغيرات البيئة أثناء بناء Docker

> **Supplementary** — complementary troubleshooting after Lecture 124; not a separate published Udemy lecture.
...
See steps below (Step 1–8). Summary:

1. Document why `.env.local` does not reach Docker build on DigitalOcean.
2. Open import chain: `/dashboard/users` → `applications.service` → `uploads.service` → `lib/storage.ts` → `requireEnv` at module load.
3. Update Dockerfile builder stage with `ARG`/`ENV` for each secret the build imports:
...
# ... mirror each ARG as ENV
```

4. Set matching variables on DigitalOcean with **Run and build time** scope.
5. Rotate any secret exposed during debugging.
6. Verify locally: `npm run build` with env vars present.
7. Redeploy development; confirm build passes `Collecting page data`.
8. **As implemented today**: Dockerfile already declares these ARGs — verify App Platform scope matches.
...
2. Inspect `/dashboard/users`.
3. Inspect `/dashboard/applications`.
